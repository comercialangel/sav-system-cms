...
export const PriceAssignment: CollectionConfig = {
  ...
  hooks: {
    ...
    //     const standardizedPricelistName = 'Precio base' // Nombre unificado
    ...
    //           data: {
    //             vehicle: vehicleId,
    //             pricelistName: standardizedPricelistName,
    //             currency: currencyId,
    ...
    //             pricelistName: { equals: standardizedPricelistName },
    //           },
    //           limit: 1,
    //           req,
    //         })
    ...
    //           throw new Error(
    //             `No existe la lista '${standardizedPricelistName}' para el vehículo ${vehicleId}`,
    //           )
    //         }
    ...
    //               price: parseFloat(doc.suggestedPrice.toFixed(2)),
    //               exchangeRate,
    //               // updatedBy: req.user?.id, (Descomenta si tu esquema lo permite)
    //             },
    //           })
    ...
    afterChange: [
      async ({ doc, req, operation }) => {
        ...
        const inventoryId =
          typeof doc.inventoryItem === 'object' ? doc.inventoryItem?.id : doc.inventoryItem
        if (!inventoryId) throw new Error('inventoryItem no contiene un ID válido')
        ...
        const vehicleId =
          typeof inventory.vehicle === 'object' ? inventory.vehicle?.id : inventory.vehicle
        if (!vehicleId) throw new Error('vehicle no contiene un ID válido')

        const currencyId = typeof doc.currency === 'object' ? doc.currency?.id : doc.currency
        if (!currencyId) throw new Error('currency no contiene un ID válido')

        const currency = await payload.findByID({ collection: 'typecurrency', id: currencyId })
        const exchangeRate = currency.codecurrency === 'USD' ? 1 : 0
        const standardizedPricelistName = 'Precio base'
        ...
        if (operation === 'create') {
          try {
            ...
            // 🚨 AQUÍ SE BORRÓ EL UPDATE AL INVENTARIO 🚨
          } catch (createError) {
            console.error('Fallo en creación de Pricelist:', createError)
            throw new Error('Fallo al crear nueva lista de precios')
          }
        }
        ...
        if (operation === 'update') {
          try {
            ...
            console.error('Error en actualización de Pricelist:', error)
            throw new Error('Fallo al actualizar lista de precios existente')
          }
        }
      },
